...
#define SEM_KEY 0x5678     // separate key for semaphore
...
void sem_wait_sysv(int semid) {
    struct sembuf sb = {0, -1, 0};  // decrement semaphore (wait)
    semop(semid, &sb, 1);
}

void sem_signal_sysv(int semid) {
    struct sembuf sb = {0, 1, 0};   // increment semaphore (signal)
    semop(semid, &sb, 1);
}
...
int main() {
    ...
    if (chat == (void *)-1) { perror("shmat"); exit(1); }
    chat->msg_count = 0;
        ...
        for (i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (FD_ISSET(sd, &readfds)) {
                ...
                if (valread <= 0) {
                    ...
                    message.msg_type = 1;
                    snprintf(message.msg_text, MAX_INPUT_SIZE,
                             "Client(fd=%d): %s", sd, buffer);
